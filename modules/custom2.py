...
def fastq_to_fasta(fastq_file, fasta_file=None):
	...
	import gzip
	if fastq_file.endswith('.gz'):
		fp=gzip.open(fastq_file, mode='rt')
	else:
		fp=open(fastq_file)
	if not fasta_file:
		fasta_file=open(fasta_file+'.fa', 'w')
	else:
...
def use_ctl_file(control_file=None):
	...
	that the control file is either (in following priority):
	1. the ctl file passed to this module elsewhere in the script
	2. if ctl file is given as second argument at command line, whatever user
	enters
	3. if no ctl file is given, identical to the script name plus '.ctl' 
	'''
	import subprocess
	import sys
	if not control_file:
...
def parse_random(prob_list, unit_random):
	'''
	given a list of probabilities associated with values that add to 1,
	reinterprets a unit random number as an output value.
	'''
	floor,ceiling=0,0
	for prob_number, pair in enumerate(prob_list):
		prob, value=pair
		ceiling+=prob
		if unit_random>=floor and unit_random<ceiling:
			return value
		floor=ceiling

def plotly_it(x_values, y_values, x_name, y_name, folder, point_names, sample_names):
	'''
	takes x_coordinates and y_coordinates as inputs. Names axes based on x_name
	and y_name. Individual points are labeled with point_names, and point colors
	are determined by sample_names.
	'''
	import plotly
	...
	import pandas as pd
	import scipy.stats
	m, b, r, linear_p_value, std_err=scipy.stats.linregress(x_values, y_values)
	m,b=round_scientific(m), round_scientific(b),
	r2, linear_p_value=round_scientific(r**2), round_scientific(linear_p_value)
	data={'x_values': x_values, 'y_values':y_values, 'point_names': point_names,
	'sample_name': sample_names}
	df = pd.DataFrame(data)
	df['trendline']=df['x_values']*m+b
	fig1=px.scatter(df, x='x_values', y='y_values', hover_data=['point_names'], color='sample_name')
	fig1['data'][0]['showlegend']=True
	fig2 = px.line(df, x='x_values', y="trendline", color_discrete_sequence=['green'])
	fig2['data'][0]['name']='trendline'
	fig2['data'][0]['showlegend']=True
	fig3 = go.Figure(data=fig1.data+fig2.data)
	title=f'm={m} b={b} r2={r2} p={linear_p_value}'
	fig3['layout']['title']=title
	fig3['layout']['xaxis']['title']=x_name
	fig3['layout']['yaxis']['title']=y_name
#	fig1.show()
#	fig5.update_yaxes(scaleanchor = "x", scaleratio = 0.9)
#	fig1.add_shape(type="line", line_color='green', line_width=2, x0=x_values[0], y0=m*x_values[0]+b, x1=x_values[-1], y1=m*x_values[-1]+b)
#	fig1.add_shape(type="line", line_color='red', line_width=2, x0=0, y0=-1, x1=20, y1=19)
	fig3.write_html(f'{folder}/{x_name}_vs_{y_name}.html')

def file_walker(input_folder):
	'''
	walks all the subdirectories 'roots' of input_folder, and returns the
	folders and files that are contained in each root.
	'''
	import os
	output_list=[]
	for root, dirs, files in os.walk(input_folder, topdown=False):
		output_list.append([root, dirs, files])
	return output_list

def find_neighbors(kmer, threshold):
	'''
	finds all neighbors of an original kmer that are within threshold distance
	of the original kmer. Does this by building a list of prefixes and their
	distances from the original kmer, and then adding one nucleotide at a time
	and updating the distances (keeping only prefixes that don't exceed the
	distance).
	'''
	options='ACGT'
	prefix_list=[['', 0]]
	for number in range(len(kmer)):
		new_prefix_list=[]
		nuc=kmer[number]
